...
contract GasPriceFeesHook is BaseHook {
    ...
    function beforeInitialize( 
        ...
        uint160,
        bytes calldata) external pure overrides returns (bytes4 ) {
        ...
        external override onlyByPoolManager returns( bytes4, beforeSwapDelta, uint24){
            uint 24 fee = getFee();
            //Si quisieramos hacer un update de las fees del LP para ua mayor duración que cada vez que 
            //haya un swap -> poolManager.updateDynamicLPFee(key,fee);
            
            //Para hacer un override del fee por swap
            uin24 feeWithFlag = fee | LPFeeLibrary.OVERRIDE_FEE_FLAG;
        ...
        function getFee() internal view returns (uint24) {
            ...
            //Si el gasPrice < movingAverageGasPrice * 0.8,
